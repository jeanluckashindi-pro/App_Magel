...
class Meeting {
  ...
  factory Meeting.fromJson(Map<String, dynamic> json) {
    ...
    try {
      ...
      dateTime = dateStr.isNotEmpty ? DateTime.parse('$dateStr $timeStr') : DateTime.now();
    } catch (e) {
  ...
  String get dynamicStatus {
    ...
    if (now.isBefore(start)) return 'Planifiée';
    if (now.isAfter(start) && now.isBefore(end)) return 'En cours';
    return 'Terminée';
  }
...
class ScannedParticipant {
  ...
  factory ScannedParticipant.fromJson(Map<String, dynamic> json) {
    return ScannedParticipant(
      id: json['id'] ?? 0,
      nom: json['id_user']?['NOM'] ?? '',
      prenom: json['id_user']?['PRENOM'] ?? '',
      email: json['id_user']?['EMAIL_PRO'] ?? '',
      photo: json['id_user']?['PHOTO'] ?? '',
      fonction: json['fonction_id']?['description'] ?? '',
      dateInsertion: DateTime.parse(json['date_insertion']?.toString() ?? DateTime.now().toString()),
    );
  }

  String get fullName => '$prenom $nom';
  String get formattedTime => DateFormat('HH:mm', 'fr_FR').format(dateInsertion);
}
...
class UserInfo {
  ...
  UserInfo({required this.profilId, required this.fonctionId, required this.adminId, required this.idUser});
...
class MeetingProvider with ChangeNotifier {
  ...
  List<ScannedParticipant> _participants = [];
  ...
  Future<void> _fetchPresenceList(int reunionId) async {
    try {
      final uri = Uri.parse('${AppConfig.baseUrl}${AppConfig.presenceEndpoint}?reunion_id=$reunionId&expand=id_user,fonction_id,profil_id');
      final token = Provider.of<AuthProvider>(navigatorKey.currentContext!, listen: false).accessToken;
      if (token == null) return;
      final response = await http.get(uri, headers: {'Authorization': 'Bearer $token'}).timeout(const Duration(seconds: AppConfig.requestTimeout));
      if (response.statusCode == 200) {
        final results = (jsonDecode(response.body)['results'] as List? ?? []);
        _participants = results.map((item) => ScannedParticipant.fromJson(item)).toList();
        notifyListeners();
      }
    } catch (e) {
      debugPrint('Erreur liste présences: $e');
    }
  }

  Future<Map<String, dynamic>> sendPresence(int reunionId, UserInfo userInfo) async {
    try {
      ...
      final body = {
        "profil_id": userInfo.profilId,
        "fonction_id": userInfo.fonctionId,
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        await _fetchPresenceList(reunionId);
        return {'success': true, 'message': 'Présence enregistrée avec succès'};
      } else if (response.statusCode == 400) {
        final errorData = jsonDecode(response.body);
        final errorMessage = errorData['message'] ?? errorData['detail'] ?? 'Erreur inconnue';
        if (errorMessage.toLowerCase().contains('existe déjà') || errorMessage.toLowerCase().contains('already exists')) {
          return {'success': false, 'message': 'Vous avez déjà marqué votre présence', 'isDuplicate': true};
        }
...
class _MeetingSelectionScreenState extends State<MeetingSelectionScreen> with TickerProviderStateMixin {
  ...
  Future<void> _loadMeetings() async {
    ...
    try {
      await provider.fetchMeetings(auth.accessToken!);
    } catch (e) {
      if (mounted) {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text('Aucune réunion', style: TextStyle(fontSize: 18, color: Colors.grey[600])),
          ],
        ),
      );
    }
        ...
        headerSliverBuilder: (context, innerBoxIsScrolled) => [
          SliverAppBar(
                          ...
                          children: [
                            ...
                            const Expanded(
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text('Magel Scanner', style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
                                  Text('Scanner les participants', style: TextStyle(color: Colors.white70, fontSize: 14)),
                                ],
                              ),
                            ),
                            IconButton(icon: const Icon(Icons.refresh, color: Colors.white), onPressed: _loadMeetings),
                          ],
                        ),
                      ],
                    ),
            ...
            bottom: TabBar(
              ...
              tabs: const [
                Tab(icon: Icon(Icons.schedule), text: 'Pour Aujourd\'hui'),
                Tab(icon: Icon(Icons.history), text: 'Récentes'),
              ],
            ),
            actions: [
              IconButton(
              ...
              return Center(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    Text('Erreur: ${provider.error}', textAlign: TextAlign.center, style: TextStyle(color: Colors.grey[700])),
                    const SizedBox(height: 16),
                ...
                Row(
                  children: [
                    Container(
                      padding: const EdgeInsets.all(12),
                      decoration: BoxDecoration(
                        gradient: LinearGradient(colors: [meeting.statusColor.withOpacity(0.1), meeting.statusColor.withOpacity(0.05)]),
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(meeting.title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold), maxLines: 2, overflow: TextOverflow.ellipsis),
                    ...
                    if (isSelected)
                      ...
                      Container(
                        padding: const EdgeInsets.all(12),
                        decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFFE53935), Color(0xFFD32F2F)]), borderRadius: BorderRadius.all(Radius.circular(12))),
                  ...
                  child: Row(
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: meeting.isToday ? const Color(0xFFFFB300).withOpacity(0.1) : Colors.blue.withOpacity(0.1),
...
class _ScannerScreenState extends State<ScannerScreen> {
  ...
  Future<void> _handleScan(BarcodeCapture capture, Meeting meeting, UserInfo userInfo) async {
    if (_hasScanned || _isSending) return;
    final code = capture.barcodes.firstWhere((b) => b.rawValue != null && b.rawValue!.isNotEmpty, orElse: () => Barcode(rawValue: '', format: BarcodeFormat.unknown)).rawValue;
    if (code == null || code.isEmpty) return;

    try {
      final scannedId = int.parse(code);
      if (scannedId != meeting.id) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('QR Code incorrect'), backgroundColor: Colors.orange));
        return;
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('QR Code invalide'), backgroundColor: Colors.red));
      return;
    }
    ...
    final result = await Provider.of<MeetingProvider>(context, listen: false).sendPresence(meeting.id, userInfo);
    final color = result['success'] == true ? const Color(0xFF43A047) : (result['isDuplicate'] == true ? Colors.orange : Colors.red);
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(result['message']), backgroundColor: color));
          ...
          children: [
            Container(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 12),
                  Expanded(child: Text('Présences (${provider.presenceCount})', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold))),
            ...
            Expanded(
              child: provider.participants.isEmpty
                  ? Center(child: Text('Aucune présence', style: TextStyle(color: Colors.grey[600])))
                  : ListView.builder(
                      padding: const EdgeInsets.all(16),
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          child: ListTile(
                            leading: CircleAvatar(backgroundImage: p.photo.isNotEmpty ? NetworkImage(p.photo) : null, child: p.photo.isEmpty ? Text(p.prenom[0]) : null),
                            title: Text(p.fullName, style: const TextStyle(fontWeight: FontWeight.bold)),
                            subtitle: Text('${p.fonction}\n${p.formattedTime}'),
  ...
  Widget build(BuildContext context) {
    ...
    if (meeting == null) return const Scaffold(body: Center(child: Text('Aucune réunion')));
  ...
  children: [
    if (_controller != null && _isScannerReady)
      MobileScanner(
        ...
        onDetect: (capture) => _handleScan(capture, meeting, authProvider.userInfo!),
      ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '${meetingProvider.presenceCount} présent(s)',
        ...
        child: const Center(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 16),
              Text('Envoi...', style: TextStyle(color: Colors.white)),
            ],
          ),
        ),
      ), // <--- CETTE PARENTHESE MANQUAIT
  ],
), // <--- ET CELLE-CI AUSSI
    );
  }
      ...
      children: [
        ...
        Center(
          child: Container(
            ...
            child: Stack(
              children: [
                ...[Alignment(-1, -1), Alignment(1, -1), Alignment(-1, 1), Alignment(1, 1)].map((a) => Align(
                      alignment: a,
                      child: Container(
                        width: 40,
                        height: 40,
                        decoration: BoxDecoration(
                          border: Border(
                            top: a.y < 0 ? const BorderSide(color: Color(0xFFE53935), width: 4) : BorderSide.none,
                            left: a.x < 0 ? const BorderSide(color: Color(0xFFE53935), width: 4) : BorderSide.none,
                            bottom: a.y > 0 ? const BorderSide(color: Color(0xFFE53935), width: 4) : BorderSide.none,
                            right: a.x > 0 ? const BorderSide(color: Color(0xFFE53935), width: 4) : BorderSide.none,
                          ),
                        ),
                      ),
                    )),
                AnimatedBuilder(
                  ...
                  builder: (context, child) => Positioned(
                    ...
                    left: 20,
                    right: 20,
                    child: Container(height: 2, decoration: BoxDecoration(gradient: const LinearGradient(colors: [Colors.transparent, Color(0xFFE53935), Color(0xFFE53935), Colors.transparent]), boxShadow: [BoxShadow(color: Color(0xFFE53935).withOpacity(0.5), blurRadius: 10)])),
                  ),
                ),
              ],
            ),
        ...
        Positioned(bottom: 160, left: 0, right: 0, child: Center(child: Container(padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12), decoration: BoxDecoration(color: Colors.black.withOpacity(0.7), borderRadius: BorderRadius.circular(20)), child: const Text('Placez le QR code dans le cadre', style: TextStyle(color: Colors.white, fontSize: 14))))),
      ],
    );
  }
}